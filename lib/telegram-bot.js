 ...
 *          https                   true/false whether to use https
 *      updates
 ...
 *          url                     URL to the webhook; if not provided `host` is required
 *          certificate             path to the https certificate for local server (required)
 *          privateKey              path to the https certificate's private key (required)
 *          max_connections         Maximum allowed number of simultaneous HTTPS connections to the
 ...
 *          allowed_updates         List the types of updates you want your bot to receive. Specify an
 *                                  empty list to receive all updates regardless of type (default).
 *          host                    hostname/local IP to listen on for webhooks
 *                                  if not provided `url` is required
 *          port                    port to listen on for webhooks, defaults to 8443
 */
...
var TelegramApi = function (params)
{
    ...
    this.sendMessage = function (params, cb)
    {
        ...
        if (typeof args.reply_markup === 'object') {
     ...
     *      allowed_updates         List the types of updates you want your bot to receive. Specify an empty list to
     *                              receive all updates regardless of type (default).
     *      max_connections         Maximum allowed number of simultaneous HTTPS connections to the webhook for
    ...
    this.setWebhook = function (params, cb)
    {
        return new Promise(function(resolve, reject)
        {
            ...
            if (params.max_connections !== undefined) args.max_connections = params.max_connections;
     ...
     *      switch_pm_text          If passed, clients will display a button with specified text that
     *                              switches the user to a private chat with the bot and sends the bot a
     *                              start message with the parameter switch_pm_parameter
     *      switch_pm_parameter     Deep-linking parameter for the /start message sent to the bot
     ...
     *      url                     URL that will be opened by the user's client. If you have created a
    ...
    if (_settings.webhook.enabled == true) {
        if (!_settings.webhook.certificate || !_settings.webhook.privateKey) {
            throw new Error('when using webhooks you must provide a certificate and private key in the configuration');
        }

        if (!_settings.webhook.host && !_settings.webhook.url) {
            throw new Error('when using webhooks you must provide either a host or a url in the configuration');
        }
