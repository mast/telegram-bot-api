...
var TelegramApi = function(params)
{
    ...
    function internalGetUpdates()
    {
        // Get bot infos
        if (! botInfos.id) {
            self.getMe(function(err, data) {
                if (! err) {
                    botInfos.id = data.id;
                    botInfos.first_name = data.first_name;
                    botInfos.username = data.username;
                }
            });
        }
        ...
        }, function(err, data)
        {
            ...
            else
            {
                ...
                data.forEach(function(item)
                {
                    ...
                    // Notify for command
                    if (item.message.text && item.message.text[0] == '/') {
                        var endPosition = item.message.text.indexOf(" ") != -1 ? item.message.text.indexOf(" ") : item.message.text.length;
                        var command = item.message.text.substring(1, endPosition);

                        // Override message text to suppress the command
                        item.message.text = item.message.text.substring(endPosition + 1, item.message.text.length);

                        self.emit('command.' + command, item.message);
                    }

                    // Notify reply
                    else if(item.message.reply_to_message !== undefined) {
                        self.emit('reply.' + item.message.reply_to_message.message_id, item.message);
                    }

                    // Notify participant join
                    else if(item.message.new_chat_participant !== undefined) {
                        self.emit('user.join', item.message);

                        if (botInfos.id && item.message.new_chat_participant.id == botInfos.id) {
                            self.emit('bot.join', item.message);
                        }
                    }

                    // Notify participant leave
                    else if(item.message.left_chat_participant !== undefined) {
                        self.emit('user.leave', item.message);

                        if (botInfos.id && item.message.left_chat_participant.id == botInfos.id) {
                            self.emit('bot.leave', item.message);
                        }
                    }
                });
            }
